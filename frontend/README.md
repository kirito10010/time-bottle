...
### 9. DataControlConsolePerformanceRecord.vue - 绩效记录

绩效记录页面提供工作汇报、加班汇报、工资记录和项目配置功能。
...
- 支持同时选择多个项目进行筛选
- 折叠标签显示，悬停查看所有选中项
- 数据绑定从单一值改为数组：`const filterProjectId = ref([])`
- 过滤逻辑使用 `includes()` 方法：`filterProjectId.value.includes(p.projectId)`
...
### 10. DataControlConsolePerformanceDashboard.vue - 绩效看板
...
- 支持同时选择多个考勤项目进行筛选
- 折叠标签显示，悬停查看所有选中项
- 数据绑定从单一值改为数组：`const selectedProject = ref([])`
- 过滤逻辑使用 `includes()` 方法：`selectedProject.value.includes(r.project)`
