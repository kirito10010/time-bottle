...
public class ExamController {
    ...
    public ResponseEntity<?> submitExam(
        ...
        Integer correctCount = (Integer) requestBody.get("correctCount");
        ...
        int pointsEarned = examService.submitExam(userId, correctCount);

        Map<String, Object> response = new HashMap<>();
        response.put("success", true);
        response.put("correctCount", correctCount);
        response.put("pointsEarned", pointsEarned);
        response.put("message", pointsEarned > 0 
            ? "恭喜！答对 " + correctCount + " 题，获得 " + pointsEarned + " 积分" 
